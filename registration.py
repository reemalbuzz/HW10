...
class Graph:
    ...
    def compute_depth(self):
        ...
        for vertex in self.vertices:
            vertex.depth = -1  # -1 means "not calculated yet"
        for v in range(len(self.vertices)):
            if self.vertices[v].depth == -1:
                self._compute_depth_helper(v)
    def _compute_depth_helper(self, v):
        """
        Helper function to recursively compute depth.
        """
        max_depth = 0
        for neighbor in self.get_adjacent_vertices(v):
            if self.vertices[neighbor].depth == -1:
                self._compute_depth_helper(neighbor)
            max_depth = max(max_depth, 1 + self.vertices[neighbor].depth)
        self.vertices[v].depth = max_depth
    def has_cycle(self):
        """
        ...
        rec_stack = [False] * len(self.adjacency_matrix)
        def dfs(v):
            if not visited[v]:
                visited[v] = True
                rec_stack[v] = True
                for neighbor in range(len(self.adjacency_matrix)):
                    if self.adjacency_matrix[v][neighbor] == 1:
    ...
    def get_registration_plan(self):
        ...
        for from_vertex in range(len(self.vertices)):
            for to_vertex in self.get_adjacent_vertices(from_vertex):
                indegree[to_vertex] += 1
        available = []
        for v in range(len(self.vertices)):
            if indegree[v] == 0:
                available.append(v)
        courses = []
        while available:
            available.sort(key=lambda v: (-self.vertices[v].depth, self.vertices[v].label))
            semester = []
            next_available = []
            for _ in range(min(4, len(available))):
                v = available.pop(0)
                semester.append(self.vertices[v].label)
                for neighbor in self.get_adjacent_vertices(v):
                    indegree[neighbor] -= 1
...
def main():
    ...
    for i, _ in enumerate(range(num_vertices)):
    ...
    for _ in range(num_edges):
        ...
        if from_label not in label_to_index:
            label_to_index[from_label] = len(label_to_index)
            graph.add_vertex(from_label)
        if to_label not in label_to_index:
            label_to_index[to_label] = len(label_to_index)
            graph.add_vertex(to_label)

        # Get the indices for the vertices
